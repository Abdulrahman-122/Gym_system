...
export default function Home() {
  ...
  const handleCheckInOut = async () => {
    ...
    try {
      await axios.post(`http://localhost:5000${route}`, {}, { withCredentials: true });
  ...
  const openTrainerPicker = async () => {
    ...
    setLoadingT(true);
    try {
      const res = await axios.get("http://localhost:5000/trainers", { withCredentials: true });
  ...
  const assignTrainer = async (trainer) => {
    setAssigning(true);
    try {
      await axios.post("http://localhost:5000/assign_trainer",
  ...
  const submitNutrition = async () => {
    try {
      await axios.post("http://localhost:5000/nutrition", {
...
{user.image_file && user.image_file !== 'default.jpg' ? (
  <img
    src={`http://localhost:5000/static/profiles/${user.image_file}`}
    alt="profile"
